...
@Suite("Date Helper Tests for start and end of the month") struct StartEndMonthTestsForRu {
    ...
    func monthStartTest(tuple: (String, String)) {
        let date = DateGenerator.date(from: tuple.0)
        let resDate = Date.Helper.startOfMonth(for: date, with: calendar)
        #expect(DateGenerator.string(from: resDate) == tuple.1)
    }
    
    @Test(
        "month end for date tests",
        arguments: [
    ...
    func monthEndTest(tuple: (String, String)) {
        let date = DateGenerator.date(from: tuple.0)
...
@Suite("Date Helper Tests for ru") struct MonthNameTestsForRu {
    ...
    @Test(
        "month name for ru locale tests",
        arguments: [
            ("03.01.2024", "январь"),
            ("23.02.2067", "февраль"),
            ("19.03.1999", "март"),
    ...
    func monthNameTest(tuple: (String, String)) {
        let date = DateGenerator.date(from: tuple.0)
        let resName = Date.Helper.monthName(for: date, with: calendar)
        #expect(resName == tuple.1)
    }
}

@Suite("Date Helper Tests for en") struct MonthNameTestsForEn {
    ...
    @Test(
        "month name for en locale tests",
        arguments: [
            ("03.01.2024", "january"),
            ("23.02.2067", "february"),
            ("19.03.1999", "march"),
